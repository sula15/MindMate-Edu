...
class StudentPersonalization:
    ...
    def _update_student_analytics(self, student_id: str) -> bool:
        """
        Update analytics for a student - FIXED VERSION
        ...
        try:
            ...
            # Learning style preferences - THIS IS THE FIX
            style_counts = {}

            # DEBUG - print all interaction learning styles
            logger.info(f"Learning styles in interactions for student {student_id}:")
            for i, interaction in enumerate(interactions):
                style = interaction.get("learning_style_id")
                logger.info(f"  Interaction {i}: {style}")

                # Skip if no learning style (None or empty string)
                if not style:
                    continue
            ...
            for module_id, mod_interactions in module_interactions.items():
                ...
                # Learning style preferences for this module - FIXED
                mod_style_counts = {}
                for interaction in mod_interactions:
                    style = interaction.get("learning_style_id")
                    if style:  # Only count if style is not None or empty
                ...
                "interaction_history": [
                    {
                        ...
                        "learning_style_id": i.get("learning_style_id")  # Ensure this field is included!
                    }
    ...
    def generate_student_report(self, student_id: str) -> Dict[str, Any]:
        """Generate a comprehensive report of student learning activity - FIXED VERSION"""
        try:
            analytics = self.get_student_analytics(student_id)
            if not analytics:
            ...
            # Learning style usage data - THIS IS THE FIX
            # Get style counts directly from interactions instead of the analytics
            style_usage = {}

            # First try to get from interaction history
            interaction_history = analytics.get("interaction_history", [])

            for interaction in interaction_history:
                style_id = interaction.get("learning_style_id")
                if style_id:  # Only count if style_id is not None or empty
                    style_usage[style_id] = style_usage.get(style_id, 0) + 1

            # If no styles found, fallback to recounting from the database
            if not style_usage:
                interactions = list(self.db[self.config.interactions_collection].find(
                    {"student_id": student_id}
                ))
            ...
            for style_id, count in style_usage.items():
                style = self.get_learning_style_by_id(style_id)
                style_name = style.name if style else (style_id.capitalize() if style_id else "Unknown")
